...
export default class ReferenceScene extends Phaser.Scene {
  ...
  constructor() {
    super(SceneIdentifier.REFERENCE_SCENE);
    this.index = 0;
    this.group = null;
    this.map = null;
    this.title = null;
  ...
  preload(): void {
    AssetManager.tiles.forEach(t => this.load.image(t.name, t.file));
  }
  ...
  create(): void {
    ...
    this.input.keyboard.on("keydown_N", () => {
      this.index += 1;
      if (this.index >= AssetManager.tiles.length) {
        this.index = 0;
      }
      this.reset();
      this.previewTileset();
  ...
  previewTileset() {
    ...
    const tileset = AssetManager.tiles[this.index];
